...
function initMenu(next) {
  next = next || document;
  let menuButton = next.querySelector(".menu-btn");
  let burgerLines = next.querySelectorAll(".menu-btn__line");
...
function initNavHover(next) {
  next = next || document;
  navItems = next.querySelectorAll("[data-nav-item]");
...
function initDownloadParallax(next) {
  next = next || document;
  let wrap = next.querySelector("[data-download-w]");
  let content = next.querySelector(".download-w");
...
function initGridRevealParallax(next) {
  ...
  triggers.forEach((trigger) => {
    let parallaxTarget = trigger.querySelector("[data-reveal-parallax]");
...
function initBlogToggle(next) {
  ...
  toggles.forEach(function (link) {
    ...
    //   const state = Flip.getState(bg);
    //   const activeLink = document.querySelector(".blog-toggle__link.active");
...
function initPressSection(next) {
  next = next || document;
  let trigger = next.querySelector("[data-press-section]");
  let prev = trigger.previousElementSibling;
  let prevInner = prev.querySelector(".main-c");
...
function initLanguage() {
  ...
  const langWrap = document.querySelector(".nav-buttons");
  if (langButton) {
  ...
  transitions: [
    {
      ...
      enter(data) {
        ...
        gsap.set(next.querySelector(".nav-inner"), { autoAlpha: 1, y: 0 });
        gsap.fromTo(
          data.next.container,
