...
function initProcessSectionText(next) {
  ...
  window.addEventListener('resize', function() {
      ...
      resizeTimeout = setTimeout(function() {
          if (window.innerWidth !== window.outerWidth) {location.reload();}
      }, 100);
  });
...
function initBlogToggle(next) {
  ...
  toggles.forEach(function (link) {
    link.addEventListener("click", function () {
      ...
      if (isNews) {
        toggleContent.style.height = `${toggleleAcademy.offsetHeight}px`;
        gsap.to(newsCards, {
          xPercent: -50,
          autoAlpha: 0,
          ease: "main",
        ...
        gsap.to(academyCards, {
          ...
          delay: 0.1,
          onComplete: () => {
            gsap.set(toggleleNews, { display: "none" }); 
          }
        });
        isNews = false;
      } else {
        gsap.set(toggleleNews, { display: "block" }); 
        toggleContent.style.height = `${toggleleNews.offsetHeight}px`;
        gsap.to(newsCards, {
          xPercent: 0,
          autoAlpha: 1,
          ease: "main",
...
function initBlogChangeCover(next){
  ...
  next.querySelectorAll('.blog-link__img img').forEach((img) => {
